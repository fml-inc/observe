...
const pluginVersion =
  typeof __FML_PLUGIN_VERSION__ !== "undefined" ? __FML_PLUGIN_VERSION__ : undefined;
...
function closestMatches(name: string, descriptors: PublicToolDescriptor[]): string[] {
...
export async function handleToolsList(opts: {
  ...
  const catW = 14;
  const header =
    "name".padEnd(nameW) + "  " + "category".padEnd(catW) + "  " + "description";
...
export async function handleToolsCall(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error(
      `Invalid JSON in ${opts.file ? "--file" : "--args"}: ${msg}`,
    );
    process.exit(1);
  }
