...
import { installCodexMcp } from "../codex-config.js";
import {
  DEFAULT_SYNC_URL,
  DEFAULT_TARGET_NAME,
  envConfigExists,
...
export async function handleInstall(
  ...
  console.log("[5/6] Setting up Codex MCP...");
  const codexResult = installCodexMcp(pluginRoot);
  if (codexResult.status === "installed") {
    console.log(`      ${codexResult.configPath}`);
  } else if (codexResult.status === "skipped") {
    console.log(`      warn: ${codexResult.detail}`);
  } else {
    console.log(`      warn: Codex MCP setup failed: ${codexResult.detail}`);
  }
  console.log();
