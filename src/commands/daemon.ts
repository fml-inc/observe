...
function printPanopticonOutput(output: string, opts: { suppressRestartHint?: boolean } = {}): void {
  const lines = output
    .trim()
    .split("\n")
    .filter(Boolean)
    .filter(
      (line) =>
        ...
        !opts.suppressRestartHint || line.trim() !== "Restart panopticon to apply.",
    );
...
export function handleSyncStart(opts: { suppressRestartHint?: boolean } = {}): void {
...
export function handleSyncStop(opts: { suppressRestartHint?: boolean } = {}): void {
