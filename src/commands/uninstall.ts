...
import { uninstallCodexMcp } from "../codex-config.js";
...
export function handleUninstall(opts: {
  ...
  // 4. Remove Codex MCP config
  const shouldHandleCodex = !targetSpecific || opts.target === "codex";
  if (shouldHandleCodex) {
    console.log("[4/6] Removing Codex MCP config...");
    const codexResult = uninstallCodexMcp();
    if (codexResult.status === "removed") {
      console.log(`      ${codexResult.detail}`);
    } else if (codexResult.status === "skipped") {
      console.log(`      ${codexResult.detail}`);
    } else {
      console.log(`      warn: Codex MCP cleanup failed: ${codexResult.detail}`);
    }
  } else {
    console.log("[4/6] Skipping Codex MCP config (target-specific uninstall)");
  }
