...
7. The spec's five-option lesson menu (Next / Try it now / Back / Jump to lesson… / Quit) maps to four explicit AskUserQuestion options (Next / Back / Try it now / Quit) plus the tool's built-in free-text "Other" for jumping to a lesson by number/title. An earlier attempt at a combined "Jump / Quit" option spawned a secondary lesson-list menu whose labels (e.g. "4 — Query your team's tools") collided with AskUserQuestion's own 1–N ordinals, showing two numbers per row; explicit Quit + free-text jump avoids both the double-numbering and the nested menu. Labels must contain no numbers.
