...
7. The Claude surface does NOT use AskUserQuestion at all. The spec's interactive menu was tried via AskUserQuestion and failed in practice three ways: (a) the model went straight to the menu without first rendering the lesson body, so the user saw navigation but no content; (b) the "Jump / Quit" option spawned a secondary lesson-list menu whose labels ("4 — Query your team's tools") collided with the picker's own 1–N ordinals (two numbers per row); (c) the picker's question/description text rendered low-contrast in some themes. Replaced with a plain-text loop: each lesson is one normal assistant message (heading + verbatim body + optional try-it + a `next/back/N/try/quit` line), navigated by typed replies. This guarantees the lesson body is always the primary visible content and removes the menu entirely.
